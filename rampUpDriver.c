...
int PID (int adjSpeed)	{			//calculating velocity
	//----------------SHOOTER VARIABLES----------------//
...
void shooterDownStartSoft(int speed){
	//int minSpeed = 20;
	//int stepDown = -2;
...
task main()		{
	while (true)	{
		...
		if (vexRT[Btn7U] == 1){
			...
			//PID(shooterSpeedGoal);
			shooterLeftSpeed = shooterSpeedGoal;
			shooterRightSpeed = shooterSpeedGoal;
		...
		if(vexRT[Btn6D] == 1)	{	// cutting off shooter speed overall
			//shooterDownStartSoft(100);			// off button switch? Fri 11/12 wow.
			motor[shooterLeft] = 0;					// fixed Sat11/13 after the competition.
